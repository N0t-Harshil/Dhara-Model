...
def executable_ratio(code: str, language: str) -> float:
    ...
    for line in lines:
        ...
        if any(stripped.startswith(m) for m in ['"""', "'''"]):
            # Single-line docstring """doc""" opens and closes on the same line
            # — toggling would mark the rest of the file as a comment.
...
def filter_code(
    ...
    if config.reject_one_liner:
        if hasattr(config, "max_one_liner_ratio") and config.max_one_liner_ratio < 1.0:
            lines = code.split(chr(10))
            long_lines = sum(1 for l in lines if len(l) >= 500)
            if long_lines / max(len(lines),1) > config.max_one_liner_ratio:
                return False, "too_many_one_liners"
        if has_one_enormous_line(code):
