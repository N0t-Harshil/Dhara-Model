...
Layer 4 is the SparseOutputSynthesizer, which implements a two-level sparse-vocabulary head: first projecting to a smaller intermediate space, then selecting the top sparsity_pct of vocabulary elements as the candidate set for scoring. Note that the forward pass still computes the full dense vocab_size x d_hidden projection for candidate scoring — top-k selects the candidate set rather than removing the dense projection. The opt-in `head_ce: topk` option (Dhara) narrows the LM loss to a candidate-set CE over the decoder's adaptive top-k indices union the target token; it reduces only the loss and backward target set, not the forward projection.

The loss computation uses sparse log-probabilities that avoid materializing the full logits tensor for the loss itself. Per-position hidden features are combined with the sandbox output and evaluated against labels using the sparse output synthesizer's compute_log_prob method over the candidate set. The dense vocab projection is still executed in the forward pass for candidate scoring; the loss is computed over the top-k candidate set plus the target, and the opt-in `head_ce: topk` (Dhara) narrows the loss and backward target set to the adaptive top-k indices union the target token.
...
The `tests/` directory contains 26 test files with a fully green suite of 292 tests. Test files cover alignment (`test_alignment.py`), configuration (`test_config.py`), data collection (`test_data_collector.py`), data pipeline (`test_data_pipeline.py`), evaluation (`test_evaluation.py`), foundation pipeline (`test_foundation_pipeline.py`), generation (`test_generation.py`), head CE modes (`test_head_ce.py`), integration (`test_integration.py`), NSLT model (`test_nslt.py`), quality (`test_quality.py`), trainer (`test_trainer.py`), validation (`test_validation.py`), plus the hardened async-pipeline tests: async pipeline overlap (`test_async_pipeline_overlap.py`), async hardening (`test_async_pipeline_hardening.py`), cache lockstep (`test_cache_lockstep.py`), cleanup pool (`test_cleanup_pool.py`), health reporter (`test_health_reporter.py`), main CLI (`test_main_cli.py`), phase-1 crash fixes (`test_phase1_crash_fixes.py`), shutdown coordinator (`test_shutdown_coordinator.py`), spec hardening (`test_spec_hardening.py`), special token alignment (`test_special_token_alignment.py`), step accounting (`test_step_accounting.py`), tokenizer acquisition (`test_tokenizer_acquisition.py`), and unit prefetch lifecycle (`test_unit_prefetch_lifecycle.py`).
